...
local function Update(self)
	local element = self.DynamiczZzIcon
...
local function Path(self, ...)
	return (self.DynamiczZzIcon.Override or Update) (self, ...)
end
...
local function Enable(self, unit)
	...
	if element and (unit == 'player' or unit == 'target') then
		...
		if element:IsObjectType('Texture') and not element:GetTexture() then
			element:SetTexture([[Interface\AddOns\ElvUI_DynamicStatusIcons\Textures\zZz - Left]])
		end
...
oUF:AddElement('DynamiczZzIcon', Path, Enable, Disable)

...
function c:Init()
    ...
    -- if GQ_OPTIONS[c.OPT_AUTOUPRANKSPELLS] then
    --     local spellCacheSize = table.getn(c:GetSpellCache());
    --     c:Println(c:GetText("Spell database indexing finished. Number of spells indexed: %s.", spellCacheSize));
    -- end

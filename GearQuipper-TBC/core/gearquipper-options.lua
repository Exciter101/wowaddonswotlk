...
local lblSaveChangesActionSlots, chSaveAlwaysActionSlots, chSaveCharMenuActionSlots, chSaveGqMenuActionSlots,
    chSaveNeverActionSlots, chDisableActionSlots;
local chIgnoreManuallySwitchedItems, chUprankSpells, chUprankSpells_AllRanks;
local chSetItemComparison, chSetItemComparison_ShowHint;

local lblVisualSettings;
local chHighlightChanges, chConfirmChangesInChat, chShowCurrentSetLabel, chShowItemTooltip, chShowSlotBackgroundColors,
    chShowBrokerTooltip;
...
function c:InitOptions()
    ...
    c.OPT_SET_ITEM_COMPARISON = "OPT_SET_ITEM_COMPARISON";
    c.OPT_SET_ITEM_COMPARISON_SHOW_HINT = "OPT_SET_ITEM_COMPARISON_SHOW_HINT";
    ...
    c.OPT_SHOW_WATERMARK = "OPT_SHOW_WATERMARK";
    ...
    c.OPTVALUE_DISABLE = "OPTVALUE_DISABLE";

    c.OPT_ACTIONBARS_SHOW_CONFIRMATION_ON_FIRST_TALENT_CHANGE =
        "OPT_ACTIONBARS_SHOW_CONFIRMATION_ON_FIRST_TALENT_CHANGE";
    ...
    if GQ_OPTIONS[c.OPT_SET_ITEM_COMPARISON] == nil then
        GQ_OPTIONS[c.OPT_SET_ITEM_COMPARISON] = true;
    end

    if GQ_OPTIONS[c.OPT_SET_ITEM_COMPARISON_SHOW_HINT] == nil then
        GQ_OPTIONS[c.OPT_SET_ITEM_COMPARISON_SHOW_HINT] = true;
    end
    ...
    if GQ_OPTIONS[c.OPT_ACTIONBARS_SHOW_CONFIRMATION_ON_FIRST_TALENT_CHANGE] == nil then
    ...
    if not scriptsPanel and GQ_OPTIONS[c.OPT_SCRIPTEDITORENABLED] then
        ...
        scriptsPanel.cancel = function(self)
            c:CheckForUnsavedScript()
        end;
        scriptsPanel.okay = function(self)
            c:CheckForUnsavedScript()
        end;
...
function c:ShowOptionsPanel()
    ...
    if not lblFeatures then
        ...
        lblFeatures:SetPoint("TOPLEFT", optionsPanel.margin,
            -(optionsPanel.elementIndex - 1) * optionsPanel.checkboxYOffset - optionsPanel.margin);
        lblFeatures:SetWidth(550);
        lblFeatures:SetHeight(23);
        lblFeatures.fontString = lblFeatures:CreateFontString("GqUiOptionElement_lblFeatures_FontString", "ARTWORK",
            "GameFontNormalLarge");
    ...
    if not lblSaveChanges then
        ...
        lblSaveChanges:SetPoint("TOPLEFT", optionsPanel.margin,
            -(optionsPanel.elementIndex - 1) * optionsPanel.checkboxYOffset - optionsPanel.margin);
        lblSaveChanges:SetWidth(100);
        lblSaveChanges:SetHeight(23);
        lblSaveChanges.fontString = lblSaveChanges:CreateFontString("GqUiOptionElement_lblSaveChanges_FontString",
    ...
    if not lblSaveChangesActionSlots then
        ...
        lblSaveChangesActionSlots:SetPoint("TOPLEFT", optionsPanel.margin, -(optionsPanel.elementIndex - 1) *
            optionsPanel.checkboxYOffset - optionsPanel.margin);
        lblSaveChangesActionSlots:SetWidth(100);
        lblSaveChangesActionSlots:SetHeight(23);
        lblSaveChangesActionSlots.fontString = lblSaveChangesActionSlots:CreateFontString(
            "GqUiOptionElement_lblSaveChangesActionSlots_FontString", "ARTWORK", "GameFontNormal");
    ...
    if not chSaveAlways then
        chSaveAlways = c:CreateOptionCheckButton(ALWAYS, optionsPanel.margin);
    ...
    if not chSaveAlwaysActionSlots then
        chSaveAlwaysActionSlots = c:CreateOptionCheckButton(ALWAYS, 280 + optionsPanel.margin);
        chSaveAlwaysActionSlots:SetScript("OnClick", function(self)
            if self:GetChecked() then
                ...
                chDisableActionSlots:SetChecked(false);
            else
                self:SetChecked(true);
            end
        end);
    ...
    if not chSaveCharMenuActionSlots then
        chSaveCharMenuActionSlots = c:CreateOptionCheckButton(c:GetText("Character menu open"),
            280 + optionsPanel.margin);
    ...
    if not chSaveGqMenuActionSlots then
        chSaveGqMenuActionSlots = c:CreateOptionCheckButton(c:GetText("GearQuipper menu open"),
            280 + optionsPanel.margin);
    ...
    if not chSaveNever then
        chSaveNever = c:CreateOptionCheckButton(NEVER, optionsPanel.margin);
    ...
    if not chSaveNeverActionSlots then
        chSaveNeverActionSlots = c:CreateOptionCheckButton(NEVER, 280 + optionsPanel.margin);
    ...
    if not chDisableActionSlots then
        chDisableActionSlots = c:CreateOptionCheckButton(c:GetText("Disable action slot management"),
            280 + optionsPanel.margin);
        chDisableActionSlots:SetScript("OnClick", function(self)
            if self:GetChecked() then
                chSaveAlwaysActionSlots:SetChecked(false);
                chSaveCharMenuActionSlots:SetChecked(false);
                chSaveGqMenuActionSlots:SetChecked(false);
    ...
    chDisableActionSlots:SetChecked(GQ_OPTIONS[c.OPT_SAVECHANGES_ACTIONSLOTS] == c.OPTVALUE_DISABLE);

    optionsPanel.elementIndex = optionsPanel.elementIndex + 1; -- spacing
    optionsPanel.elementIndex = optionsPanel.elementIndex + 1; -- spacing

    if not chIgnoreManuallySwitchedItems then
        chIgnoreManuallySwitchedItems = c:CreateOptionCheckButton(c:GetText(
            "Ignore manually switched items for the next event triggered set switch"));
        optionsPanel.elementIndex = optionsPanel.elementIndex + 1;
    end
    ...
    if not chUprankSpells_AllRanks then
        chUprankSpells_AllRanks = c:CreateOptionCheckButton(c:GetText(
            "Uprank from any rank (will uprank any lower rank to max rank)"), optionsPanel.margin);
    ...
    if not chSetItemComparison then
        chSetItemComparison = c:CreateOptionCheckButton(c:GetText("Set item comparison tooltips (L-Alt)"));
        chSetItemComparison:SetScript("OnClick", function(self)
            chSetItemComparison_ShowHint:SetEnabled(self:GetChecked());
            if self:GetChecked() then
                chSetItemComparison_ShowHint.fontString:SetTextColor(unpack(
                    chSetItemComparison_ShowHint.fontString.defaultColor));
            else
                chSetItemComparison_ShowHint:SetChecked(false);
                chSetItemComparison_ShowHint.fontString:SetTextColor(0.7, 0.7, 0.7);
            end
        end);
        optionsPanel.elementIndex = optionsPanel.elementIndex + 1;
    end
    chSetItemComparison:SetChecked(GQ_OPTIONS[c.OPT_SET_ITEM_COMPARISON]);

    if not chSetItemComparison_ShowHint then
        chSetItemComparison_ShowHint = c:CreateOptionCheckButton(
            c:GetText("Show hint on item tooltip (\"%s\")", c:GetText("Press L-Alt to cycle through sets.")),
            optionsPanel.margin);
        chSetItemComparison_ShowHint.fontString.defaultColor = {chSetItemComparison_ShowHint.fontString:GetTextColor()};
        optionsPanel.elementIndex = optionsPanel.elementIndex + 1;
    end
    chSetItemComparison_ShowHint:SetChecked(GQ_OPTIONS[c.OPT_SET_ITEM_COMPARISON_SHOW_HINT]);

    if not lblSwitchDelay then
        lblSwitchDelay = CreateFrame("Frame", "GqUiOptionElement_lblSwitchDelay", optionsPanel);
        lblSwitchDelay:SetPoint("TOPLEFT", optionsPanel.margin, -(optionsPanel.elementIndex - 1) *
            optionsPanel.checkboxYOffset - optionsPanel.margin - 10);
        lblSwitchDelay:SetWidth(100);
        lblSwitchDelay:SetHeight(23);
        lblSwitchDelay.fontString = lblSwitchDelay:CreateFontString("GqUiOptionElement_lblSwitchDelay_FontString",
            "ARTWORK", "GameFontNormal");
    ...
    if not lblSwitchDelayHint then
        ...
        lblSwitchDelayHint:SetPoint("TOPLEFT", 375, -(optionsPanel.elementIndex - 1) * optionsPanel.checkboxYOffset -
            optionsPanel.margin - 10);
        lblSwitchDelayHint:SetWidth(100);
        lblSwitchDelayHint:SetHeight(23);
        lblSwitchDelayHint.fontString = lblSwitchDelayHint:CreateFontString(
            "GqUiOptionElement_lblSwitchDelayHint_FontString", "ARTWORK", "GameFontNormal");
        lblSwitchDelayHint.fontString:SetPoint("TOPLEFT", 3, 0);
        lblSwitchDelayHint.fontString:SetJustifyH("LEFT");
    end
    lblSwitchDelayHint.fontString:SetText(c:GetText(
        "(Increase if you experience issues with\n automated event switches)"));
    ...
    if not lblVisualSettings then
        ...
        lblVisualSettings:SetPoint("TOPLEFT", optionsPanel.margin, -(optionsPanel.elementIndex - 1) *
            optionsPanel.checkboxYOffset - optionsPanel.margin);
        lblVisualSettings:SetWidth(550);
        lblVisualSettings:SetHeight(23);
        lblVisualSettings.fontString = lblVisualSettings:CreateFontString(
            "GqUiOptionElement_lblVisualSettings_FontString", "ARTWORK", "GameFontNormalLarge");
    ...
    if not chConfirmChangesInChat then
        chConfirmChangesInChat = c:CreateOptionCheckButton(c:GetText("Confirm changes in chat (gear and action slots)"));
        optionsPanel.elementIndex = optionsPanel.elementIndex + 1;
    end
    ...
    if not btnReset then
        ...
        btnReset:SetPoint("TOPRIGHT", -10,
            -optionsPanel.elementIndex * optionsPanel.checkboxYOffset - optionsPanel.margin);
...
function c:SetCheckboxEnabled(frame, value, uncheckIfDisabled)
    if frame and frame.fontString then
        if uncheckIfDisabled and not value then
            frame:SetChecked(false);
        end
        frame:SetEnabled(value);

        if not frame.fontString.defaultColor then
            frame.fontString.defaultColor = {frame.fontString:GetTextColor()};
        end

        if value then
            frame.fontString:SetTextColor(unpack(frame.fontString.defaultColor));
        else
...
function c:CreateOptionCheckButton(text, xOffset)
    ...
    local element = CreateFrame("CheckButton", "GqUiOptionElement_" .. optionsPanel.elementIndex, optionsPanel,
        "ChatConfigCheckButtonTemplate");
    element:SetPoint("TOPLEFT", 10 + xOffset,
        -(optionsPanel.elementIndex - 1) * optionsPanel.checkboxYOffset - optionsPanel.margin);
    element.fontString = element:CreateFontString("GqUiOptionElement_" .. optionsPanel.elementIndex .. "_FontString",
        "ARTWORK", "GameFontNormal");
...
function c:CreateOptionsSlider(min, max, func, xOffset, yOffset, width, valueStep, minText, maxText)
    ...
    local element = CreateFrame("Slider", "GqUiOptionElement_" .. optionsPanel.elementIndex, optionsPanel,
        "OptionsSliderTemplate");
    element.frameType = "Slider";
    element:SetPoint("TOPLEFT", 10 + xOffset,
        -(optionsPanel.elementIndex - 1) * optionsPanel.checkboxYOffset - optionsPanel.margin - yOffset);
...
function c:SaveOptions(self)
    if c.initFinished then
        ...
        GQ_OPTIONS[c.OPT_SET_ITEM_COMPARISON] = chSetItemComparison:GetChecked();
        GQ_OPTIONS[c.OPT_SET_ITEM_COMPARISON_SHOW_HINT] = chSetItemComparison_ShowHint:GetChecked();
        ...
        elseif chSaveNeverActionSlots:GetChecked() then
...
function c:IsActionSlotManagementEnabled()
    return GQ_OPTIONS[c.OPT_SAVECHANGES_ACTIONSLOTS] and GQ_OPTIONS[c.OPT_SAVECHANGES_ACTIONSLOTS] ~= c.OPTVALUE_DISABLE;
end
...
local function GetScriptListEntry(uiIndex)
    if not listScriptEntries[uiIndex] then
        local scriptEntry = CreateFrame("Button", "GqUiOptionElement_Scripts_Entry" .. uiIndex, scriptListContentFrame,
            "GqButtonTemplate");
        ...
        scriptEntry.fontString = scriptEntry:CreateFontString(
            "GqUiOptionElement_Scripts_Entry_" .. uiIndex .. "_FontString", "ARTWORK", "GameFontWhite");
...
function c:ShowScriptEditor()
    if not lblExistingScripts then
        ...
        lblExistingScripts.fontString = lblExistingScripts:CreateFontString(
            "GqUiOptionElement_lblExistingScripts_FontString", "ARTWORK", "GameFontNormal");
    ...
    if not scriptListScrollFrameBorder then
        scriptListScrollFrameBorder = CreateFrame("Frame", "GearQuipper_Options_ScriptEditor_ListScrollFrameBorder",
            scriptsPanel);
    ...
    if not scriptListScrollFrame then
        scriptListScrollFrame = CreateFrame("ScrollFrame", "GearQuipper_Options_ScriptEditor_ListScrollFrame",
            scriptListScrollFrameBorder);
    ...
    if not scriptListScrollFrameSlider then
        scriptListScrollFrameSlider = CreateFrame("Slider", "GearQuipper_Options_ScriptEditor_ListScrollFrameSlider",
            scriptListScrollFrame);
        ...
        scriptListScrollFrameSlider:SetBackdrop({
            ...
            edgeFile = "Interface\\Buttons\\UI-SliderBar-Background",
            tile = true,
            tileSize = 8,
            edgeSize = 8,
            insets = {
    ...
    if not scriptListContentFrame then
        scriptListContentFrame = CreateFrame("Frame", "GearQuipper_Options_ScriptEditor_scriptListContentFrame",
            scriptListScrollFrameBorder);
        scriptListContentFrame:SetPoint("TOPLEFT", 10, -10);
        scriptListContentFrame:SetPoint("BOTTOMRIGHT", -10, 10);
        scriptListContentFrame:SetWidth(
            scriptListScrollFrameBorder:GetWidth() - scriptListScrollFrameSlider:GetWidth() - 10);
    ...
    if not lblScriptName then
        ...
        lblScriptName:SetPoint("TOPLEFT", scriptsPanel.margin + scriptListScrollFrameBorder:GetWidth() + 8,
            -scriptsPanel.margin - defaultYOffset);
        lblScriptName:SetWidth(100);
        lblScriptName:SetHeight(23);
        lblScriptName.fontString = lblScriptName:CreateFontString("GqUiOptionElement_lblScriptName_FontString",
            "ARTWORK", "GameFontNormal");
    ...
    if not txtScriptName then
        txtScriptName = CreateFrame("EditBox", "GearQuipper_Options_ScriptEditor_TxtScriptName", scriptsPanel,
            "InputBoxTemplate");
        txtScriptName:SetPoint("TOPLEFT", scriptsPanel.margin + scriptListScrollFrameBorder:GetWidth() + 8 +
            lblScriptName:GetWidth() + 44, -scriptsPanel.margin - defaultYOffset + 2);
        ...
        txtScriptName:SetScript("OnEscapePressed", function()
            txtScriptName:ClearFocus()
        end);
    ...
    if not lblScriptStatus then
        ...
        lblScriptStatus:SetPoint("TOPLEFT",
            scriptListScrollFrameBorder:GetWidth() + 8 + lblScriptName:GetWidth() + 44 + txtScriptName:GetWidth() + 15,
            -scriptsPanel.margin - defaultYOffset);
        lblScriptStatus:SetWidth(100);
        lblScriptStatus:SetHeight(23);
        lblScriptStatus.fontString = lblScriptStatus:CreateFontString("GqUiOptionElement_lblScriptStatus_FontString",
            "ARTWORK", "GameFontNormal");
    ...
    if not lblScriptText then
        ...
        lblScriptText:SetPoint("TOPLEFT", scriptsPanel.margin + scriptListScrollFrameBorder:GetWidth() + 8,
            -scriptsPanel.margin - defaultYOffset * 2);
        lblScriptText:SetWidth(100);
        lblScriptText:SetHeight(23);
        lblScriptText.fontString = lblScriptText:CreateFontString("GqUiOptionElement_lblScriptName_FontString",
            "ARTWORK", "GameFontNormal");
    ...
    if not txtScriptFrameBorder then
        txtScriptFrameBorder = CreateFrame("Frame", "GearQuipper_Options_ScriptEditor_txtScriptFrameBorder",
            scriptsPanel);
        txtScriptFrameBorder:SetPoint("TOPLEFT", scriptsPanel.margin + scriptListScrollFrameBorder:GetWidth() + 5,
            -scriptsPanel.margin - defaultYOffset * 3);
    ...
    if not txtScriptText then
        ...
        txtScriptText:SetScript("OnEscapePressed", function()
            txtScriptText:ClearFocus()
        end);
    ...
    if not lblSelectScript then
        ...
        lblSelectScript.fontString = lblSelectScript:CreateFontString("GqUiOptionElement_lblExistingScripts_FontString",
            "ARTWORK", "GameFontNormal");
        ...
        lblSelectScript.fontString:SetText(c:GetText(
            "Please select a script or create a new one using the button below."));
    ...
    if not btnNewScript then
        btnNewScript = CreateFrame("Button", "GearQuipper_Options_ScriptEditor_BtnNewScript", scriptsPanel,
            "GqButtonTemplate");
        btnNewScript:SetPoint("BOTTOMLEFT", scriptsPanel.margin + 15, scriptsPanel.margin);
        btnNewScript:SetWidth(120);
        btnNewScript:SetScript("OnClick", function()
            NewScript()
        end);
        btnNewScript.fontString = btnNewScript:CreateFontString(
            "GearQuipper_Options_ScriptEditor_BtnNewScript_FontString", "ARTWORK", "GameFontNormal");
    ...
    if not btnDeleteScript then
        btnDeleteScript = CreateFrame("Button", "GearQuipper_Options_ScriptEditor_BtnDeleteScript", scriptsPanel,
            "GqButtonTemplate");
        btnDeleteScript:SetPoint("BOTTOMLEFT", scriptsPanel.margin + 165, scriptsPanel.margin);
        btnDeleteScript:SetWidth(120);
        btnDeleteScript:SetScript("OnClick", function()
            DeleteScript()
        end);
        btnDeleteScript.fontString = btnDeleteScript:CreateFontString(
            "GearQuipper_Options_ScriptEditor_BtnDeleteScript_FontString", "ARTWORK", "GameFontNormal");
    ...
    if not btnTestScript then
        btnTestScript = CreateFrame("Button", "GearQuipper_Options_ScriptEditor_BtnTestScript", scriptsPanel,
            "GqButtonTemplate");
        ...
        btnTestScript.fontString = btnTestScript:CreateFontString(
            "GearQuipper_Options_ScriptEditor_BtnTestScript_FontString", "ARTWORK", "GameFontNormal");
    ...
    if not btnSaveScript then
        btnSaveScript = CreateFrame("Button", "GearQuipper_Options_ScriptEditor_BtnSaveScript", scriptsPanel,
            "GqButtonTemplate");
        btnSaveScript:SetPoint("BOTTOMRIGHT", -scriptsPanel.margin, scriptsPanel.margin);
        btnSaveScript:SetWidth(120);
        btnSaveScript:SetScript("OnClick", function()
            SaveScript()
        end);
        btnSaveScript.fontString = btnSaveScript:CreateFontString(
            "GearQuipper_Options_ScriptEditor_BtnSaveScript_FontString", "ARTWORK", "GameFontNormal");
...
function c:CheckForUnsavedScript()
    if currentEntry and currentEntry.script and txtScriptText and currentEntry.script[c.FIELD_TEXT] ~=
        txtScriptText:GetText() then

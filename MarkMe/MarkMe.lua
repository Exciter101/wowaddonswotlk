...
mainFrame = CreateFrame("Frame", "mainFrame", UIParent, BackdropTemplateMixin and "BackdropTemplate")
...
local iconFrame = CreateFrame("Frame", "iconFrame", mainFrame, BackdropTemplateMixin and "BackdropTemplate")
...
iconFrame:SetScript("OnMouseDown", function(self,button) if (button=="LeftButton" and MMSettings.lock_frame ~= true) then mainFrame:StartMoving(); self.isMoving = true; end end)
iconFrame:SetScript("OnMouseUp", function(self) if (self.isMoving) then mainFrame:StopMovingOrSizing(); self.isMoving = false; end end)


------------- Readycheck Frame -------------

local ReadycheckFrame = CreateFrame("Frame", "ReadycheckFrame", mainFrame, BackdropTemplateMixin and "BackdropTemplate")
...
ReadycheckFrame:SetPoint("RIGHT", iconFrame, "LEFT")
ReadycheckFrame:SetScript("OnMouseDown", function(self,button) if (button=="LeftButton" and MMSettings.lock_frame ~= true) then mainFrame:StartMoving(); self.isMoving = true; end end)
ReadycheckFrame:SetScript("OnMouseUp", function(self) if (self.isMoving) then mainFrame:StopMovingOrSizing(); self.isMoving = false; end end)
...
local AnnounceFrame = CreateFrame("Frame", "AnnounceFrame", mainFrame, BackdropTemplateMixin and "BackdropTemplate")
...
Readycheck:SetScript("OnMouseDown", function(self,button) if (button=="LeftButton" and MMSettings.lock_frame ~= true) then mainFrame:StartMoving() end end)
Readycheck:SetScript("OnMouseUp", function(self) mainFrame:StopMovingOrSizing() end)
...
announceIcon:SetScript("OnClick", function(self) if (enable_announcer) then AnnounceOP() end end)
...
getglobal(FadeSlider:GetName().."Text"):SetText(MM .." fade: "..math.floor((MMSettings.fade_num*100)).."%")	
FadeSlider:SetValue(MMSettings.fade_num)
FadeInit()
CogInit()
end
...
local FadeSlider = CreateFrame("Slider", "FadeSlider", MarkMeOptions.panel, "OptionsSliderTemplate")
FadeSlider:SetPoint("LEFT", Use_Fade, "BOTTOMLEFT",0,-35)
FadeSlider:SetSize(180,16)
FadeSlider:SetMinMaxValues(0.1,1)
FadeSlider:SetValue(100)
FadeSlider:SetValueStep(0.05)
FadeSlider:SetOrientation("HORIZONTAL")
FadeSlider:SetScript("OnValueChanged", function(self) MM_Use_fade_num(Self) end)
FadeSlider:SetScript("OnMouseUp", function(self) MMSettings.fade_num = (FadeSlider:GetValue()) end)
FadeSlider:SetScript("OnLoad", function(self) MM_Use_fade_num(self) end)
 getglobal(FadeSlider:GetName() .. 'Low'):SetText('1%');
 getglobal(FadeSlider:GetName() .. 'High'):SetText('100%');


local ScaleSlider = CreateFrame("Slider", "ScaleSlider", MarkMeOptions.panel, "OptionsSliderTemplate")
ScaleSlider:SetPoint("LEFT", FadeSlider, "BOTTOMLEFT",0,-35)
...
MMSettings.fade_num = (FadeSlider:GetValue());
getglobal(FadeSlider:GetName().."Text"):SetText(MM .." fade: "..math.floor((MMSettings.fade_num*100)).."%")		
FadeInit()
end
